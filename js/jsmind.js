...
(function($w){
    ...
    jm.node = function(sId,iIndex,sTopic,oData,bIsRoot,oParent,eDirection,bExpanded,sUnixTime){
        ...
        //if(typeof sUnixTime != 'number'){logger.error('invalid time stamp');return;}
        this.id = sId;
        this.index = iIndex;
        this.topic = sTopic;
        this.data = oData || {};
        ...
        add_node:function(parent_node, nodeid, topic, data, idx, direction, expanded){
            ...
            if(!!parent_node){
                ...
                if(parent_node.isroot){
                    ...
                    node = new jm.node(nodeid,nodeindex,topic,data,false,parent_node,d,expanded,timestamp);
                }else{
                    node = new jm.node(nodeid,nodeindex,topic,data,false,parent_node,parent_node.direction,expanded,timestamp);
                }
    ...
    jm.format = {
        node_tree:{
            example:{
                ...
                "format":"node_tree",
                "data":{"id":"root","topic":"Your Project"}
            },
        ...
        node_array:{
            example:{
                ...
                "format":"node_array",
                "data":[
                    {"id":"root","topic":"Your Project", "isroot":true}
                ]
            },
...
// ============= data provider =============================================
    ...
    jm.shortcut_provider.prototype = {
        init : function(){
            ...
            this.handles['hello'] = this.handle_hello;
        ...
        handle_hello: function(_jm,e){
            var selected_node = _jm.get_node("powerful2");
            if(!!selected_node){
                var nodeid = jm.util.uuid.newid();
                var node = _jm.insert_node_after(selected_node, nodeid, 'New Node');
                _jm.update_node(nodeid,"hello world.");
                console.log(node);
                if(!!node){
                    _jm.select_node(nodeid);
                    //  _jm.begin_edit(nodeid);
